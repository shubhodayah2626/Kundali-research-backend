...
const { calculateAllScoresFromManualPositions, LPI_DOMAINS } = require('./scoring');
...
const kundaliSchema = new mongoose.Schema({
  ...
  lpiAnswers: mongoose.Schema.Types.Mixed,
...
function generateCeiLpiPdf(res, { name, dateOfBirth, timeOfBirth, location, scores, scoringDetails }) {
  ...
  doc.fontSize(7.5).fillColor('#555555').text('LPI = sum of 9 life-domain scores (5 questions x 2 points each) — answered by the father').fillColor('#000000');
  doc.moveDown(0.2);
  if (sd.lpi && sd.lpi.components) {
    const rows = [['LPI Domain', 'Score', '/10']];
    const domainLabels = sd.lpi.domainLabels || {};
    Object.entries(sd.lpi.components).forEach(([k, v]) => rows.push([domainLabels[k] || k, v ?? 'N/A', '10']));
    drawTable(doc, rows, { colWidths: [340, 70, 50] });
  }
...
/**
 * MAIN FLOW: father submits planetary positions + LPI questionnaire
 * answers in one form -> this computes CEI + LPI AND streams the
 * single combined PDF directly as the response -> browser downloads
 * it immediately. No separate "results page" click needed.
 * Also saves a record in the background (non-blocking) for history.
 */
app.post('/api/kundali/generate', async (req, res) => {
  try {
    const { name, dateOfBirth, timeOfBirth, location, planetaryPositions, lpiAnswers } = req.body;
    if (!name) return res.status(400).json({ error: 'Name is required' });
    if (!planetaryPositions) return res.status(400).json({ error: 'Planetary positions are required' });

    const { scores, scoringDetails } = calculateAllScoresFromManualPositions(planetaryPositions, lpiAnswers);
    ...
    new Kundali({
      ...
      location: location || {}, planetaryPositions, lpiAnswers, scores, scoringDetails
...
// Returns the LPI questionnaire structure (9 domains x 5 questions) for the frontend to render
app.get('/api/lpi-questions', (req, res) => {
  res.json(LPI_DOMAINS);
});
