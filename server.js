...
function drawPageChrome(doc) {
  ...
  doc.fontSize(7.5).fillColor(GOLD)
    .text('ॐ पीएचडी शोध परियोजना • ब्रह्माण्डीय ऊर्जा सूचकांक मैट्रिक्स / जीवन निष्पादन सूचकांक मैट्रिक्स • अनुराग मिश्रा',
      18, doc.page.height - 32, { width: doc.page.width - 36, align: 'center' });
  doc.fillColor(INK);
}
...
function renderScorePage(doc, content, { rows, score, maxScore, columnLabel }) {
  newPage(doc);
  ...
  const tableRows = rows.map(r => [r.label, '2', r.pending ? 'लंबित (Pending)' : String(r.score)]);
...
function renderCoverPage(doc, content, headerInfo) {
  ...
  // callers pass a flag via headerInfo.isFirst to control this.
  if (!headerInfo || !headerInfo.isFirst) newPage(doc); else drawPageChrome(doc);
  doc.y = 60;
...
function generateCeiLpiPdf(res, { name, dateOfBirth, timeOfBirth, location, scores, scoringDetails }) {
  ...
  LPI_DOMAIN_ORDER.forEach(key => {
    ...
    renderScorePage(doc, content, { rows, score, maxScore: 10, columnLabel: 'प्राप्त अंक' });
  });
