...
const { computeAshtakvarga } = require('./ashtakvargaCalc');
...
/**
 * Grahan Dosha: father's explanation -- "राहु केतु अकेले बैठे इसलिए किसी पर
 * ग्रहण नहीं लग रहा है" (Rahu/Ketu sitting alone means no planet is
 * eclipsed). Implemented as: Rahu or Ketu conjunct (same sign as) any of
 * the 7 classical planets.
 */
function checkGrahanDosha(planetaryPositions) {
  const classicalPlanets = ['sun', 'moon', 'mars', 'mercury', 'jupiter', 'venus', 'saturn'];
  const rahuRashi = planetaryPositions.rahu.rashi;
  const ketuRashi = planetaryPositions.ketu.rashi;
  return classicalPlanets.some(p => planetaryPositions[p] && (planetaryPositions[p].rashi === rahuRashi || planetaryPositions[p].rashi === ketuRashi));
}

/**
 * Nadi Dosha: DOCUMENTED PLACEHOLDER -- classically this is a
 * COMPATIBILITY check between two charts' Moon-nakshatra Nadi category
 * (Aadi/Madhya/Antya), used in marriage matching (Ashtakoot Milan). It
 * does not have an obvious single-chart definition, so this always
 * returns false pending clarification from the father on what he means
 * by it in a single-natal-chart CEI context.
 */
function checkNadiDosha() {
  return false; // TODO: clarify single-chart definition with father
}

/**
 * Pitra Dosha: DOCUMENTED SIMPLIFICATION -- classical definitions vary widely
 * across traditions. Used here: Sun conjunct Rahu/Ketu, OR Rahu/Ketu occupying
 * the 9th house from Lagna (both are commonly-cited simplified triggers).
 */
function checkPitraDosha(planetaryPositions, lagnaRashi) {
  const sun = planetaryPositions.sun;
  const sunConjunctShadow = sun && (sun.rashi === planetaryPositions.rahu.rashi || sun.rashi === planetaryPositions.ketu.rashi);
  const ninthHouseRashi = RASHI_ORDER[(RASHI_ORDER.indexOf(lagnaRashi) + 8) % 12];
  const shadowIn9th = planetaryPositions.rahu.rashi === ninthHouseRashi || planetaryPositions.ketu.rashi === ninthHouseRashi;
  return !!(sunConjunctShadow || shadowIn9th);
}
...
async function calculateFromBirthDetails(birthDetails) {
  ...
  const [planetsRaw, shadbalaRaw, bhavaBalaRaw] = await Promise.all([
  ...
  // ---- Ashtakvarga: computed locally (verified exact match against AstroSage's
  // real numbers for Rahul Gandhi, and against B.V. Raman's own textbook worked
  // example) -- NOT from the API, since AstrologyAPI's sarvashtak endpoint was
  // found to use a different classical bindu-contribution convention that
  // disagreed with AstroSage by 5-8 points on several signs. ----
  const ashtakvarga = computeAshtakvarga(planetaryPositions);

  // ---- Bhava Bala: reshape into { houseNumber: { totalBhavaBalaRupas, strengthPercent } } ----
  const bhavaBala = {};
  (bhavaBalaRaw && bhavaBalaRaw.houses ? bhavaBalaRaw.houses : []).forEach(h => {
    if (h.id == null) return;
    bhavaBala[h.id] = {
      totalBhavaBalaRupas: h.total_bhavabala_rupa,
      strengthPercent: h.strength_percent_of_baseline
    };
  });

  // ---- Doshas (computed locally from the API's real positions -- same logic as before) ----
  const doshas = {
    // Father's confirmed EL dosha list (voice note): Kaal Sarp, Manglik
    // (Mangal), Pitra, Nadi, and Grahan Dosha. Father's explanation for
    // Manglik confirmed it's checked from LAGNA only (Mars in the 9th
    // house from Lagna is why Rahul Gandhi has none -- house 9 isn't a
    // classical Manglik house; checking from Moon too would wrongly
    // flag it, since house 7 from Moon IS one, but father didn't
    // reference the Moon-based check at all).
    kaalSarpDosha: checkKaalSarpDosha(planetaryPositions),
    mangalDosha: checkMangalDosha(planetaryPositions.mars.rashi, lagnaRashi),
    pitraDosha: checkPitraDosha(planetaryPositions, lagnaRashi),
    nadiDosha: checkNadiDosha(),
    grahanDosha: checkGrahanDosha(planetaryPositions)
  };
  ...
  return {
    ...
    nakshatraLord,
    moonNakshatraName,
    source: 'astrologyapi.com + local Ashtakvarga',
    calculationNotes: [
      'Planetary positions, Shadbala, and Bhava Bala computed by AstrologyAPI.com (Swiss Ephemeris-grade).',
      'Ashtakvarga computed LOCALLY (see ashtakvargaCalc.js) using verified classical Parashara bindu tables -- AstrologyAPI\'s sarvashtak endpoint was found to use a different convention that disagreed with AstroSage by 5-8 points on several signs.',
      'Validated against the real AstroSage chart for Rahul Gandhi -- every planet position matched to within a couple arcminutes, and locally-computed Ashtakvarga matched AstroSage\'s real numbers exactly.',
      'EL (Energy Loss) doshas per father\'s confirmation: Kaal Sarp, Manglik (Mangal, checked from Lagna only), Pitra, Nadi (placeholder pending clarification), and Grahan Dosha.'
    ]
  };
}
