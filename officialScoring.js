...
/**
 * SB = षड्बल (Shadbala). Literal spec criteria (each planet's average
 * across the 7 classical grahas, compared to a documented "good"
 * threshold — the worksheet says "अच्छा"/"good" without a numeric
 * cutoff, so each threshold below is set at roughly half of that
 * sub-component's typical maximum, consistent with how AV's cutoffs
 * work elsewhere in this file):
 *  1. स्थानबल अच्छा (Sthana Bala good) — avg totalSthanaBala >= 90 (of ~180 max)
 *  2. दिग्बल अच्छा (Dig Bala good) — avg totalDigBala >= 30 (of 60 max)
 *  3. कालबल अच्छा (Kaal Bala good) — avg totalKaalBala >= 195 (of ~390 max)
 *  4. चेष्टाबल अच्छा (Cheshta Bala good) — avg totalChestaBala >= 30 (of 60 max)
 *  5. नैसर्गिक/रूपबल अच्छा (Naisargika Bala good) — avg totalNaisargikaBala >= 30 (of 60 max)
 */
...
// Deva-gana (classically "benefic-natured") Nakshatras — used for N's
// "नक्षत्र शुभ प्रकृति का" (nakshatra of benefic nature) criterion.
// DOCUMENTED SIMPLIFICATION: classifies by the standard 3-gana system
// (Deva/Manushya/Rakshasa); only Deva-gana counts as "benefic nature"
// here since the spec gives no finer rule.
const DEVA_GANA_NAKSHATRAS = ['Ashwini', 'Mrigashira', 'Punarvasu', 'Pushya', 'Hasta', 'Swati', 'Anuradha', 'Shravana', 'Revati'];

// Shadow-planet (Rahu/Ketu) dignity — classical texts vary by
// tradition; this uses the commonly-cited Jagannatha Hora convention
// (Rahu exalted Gemini/debilitated Sagittarius, Ketu exalted
// Sagittarius/debilitated Gemini) — documented choice, since the
// worksheet doesn't specify which tradition to follow.
const SHADOW_EXALTATION = { rahu: 'Gemini', ketu: 'Sagittarius' };
const SHADOW_DEBILITATION = { rahu: 'Sagittarius', ketu: 'Gemini' };

/**
 * N = नक्षत्र (Nakshatra). Literal spec criteria:
 *  1. नक्षत्र स्वामी बलवान (Nakshatra lord strong) — Shadbala ratio >= 1
 *     for the 7 grahas; for Rahu/Ketu (no Shadbala), a kendra/trikona
 *     house placement is used as the "strong" proxy instead.
 *  2. नक्षत्र स्वामी शुभ स्थान में (Nakshatra lord in auspicious house) — kendra/trikona from Lagna
 *  3. नक्षत्र स्वामी पाप प्रभाव से मुक्त (Nakshatra lord free from malefic influence)
 *  4. नक्षत्र शुभ प्रकृति का (Nakshatra itself of benefic nature) — Deva-gana (see above)
 *  5. नक्षत्र स्वामी उच्च/स्वराशि (Nakshatra lord exalted/own sign) — for Rahu/Ketu, uses SHADOW dignity table above
 */
function scoreN(shadbala, nakshatraLord, planetaryPositions, moonNakshatraName) {
  ...
  if (isGrahaLord) {
    ...
    score += 2; // shadow-planet "strength" proxy: kendra/trikona placement
  }
...
/**
 * H = शरीर एवं 12 भाव (Body & 12 houses). Literal spec criteria:
 *  1. 1–3 भाव मजबूत (houses 1-3 strong)
 *  2. 4–6 भाव मजबूत (houses 4-6 strong)
 *  3. 7–9 भाव मजबूत (houses 7-9 strong)
 *  4. 10–12 भाव मजबूत (houses 10-12 strong)
 *  5. शरीर कारक ग्रह मजबूत (body-significator planet strong)
 * "Strong" = avg totalBhavaBalaRupas across the group >= 7.5 (half of
 * the ~15-rupa scale already used elsewhere in this file). The body
 * karaka (significator) is taken as the Sun, per classical convention
 * (Surya = atma-karaka / vitality significator) — documented choice,
 * since the worksheet doesn't name which planet.
 */
function scoreH(bhavaBala, shadbala) {
  if (!bhavaBala) return null;
  const avgGroup = houses => {
    const vals = houses.map(h => bhavaBala[h] && bhavaBala[h].totalBhavaBalaRupas).filter(v => v != null);
    if (!vals.length) return null;
    return vals.reduce((a, b) => a + b, 0) / vals.length;
  };

  let score = 0;
  const g1 = avgGroup([1, 2, 3]); if (g1 != null && g1 >= 7.5) score += 2;
  const g2 = avgGroup([4, 5, 6]); if (g2 != null && g2 >= 7.5) score += 2;
  const g3 = avgGroup([7, 8, 9]); if (g3 != null && g3 >= 7.5) score += 2;
  const g4 = avgGroup([10, 11, 12]); if (g4 != null && g4 >= 7.5) score += 2;
  const sunRatio = shadbala && shadbala.sun && shadbala.sun.ratio;
  if (sunRatio != null && sunRatio >= 1) score += 2;

  return score;
}

/**
 * P = पंचमहाभूत (Panchamahabhuta / 5 elements). Literal spec criteria:
 * each of Fire/Earth/Air/Water balanced, plus Akash (Ether/Space)
 * balanced. DOCUMENTED SIMPLIFICATION: "balanced" for the 4 zodiacal
 * elements = 1 to 4 of the 9 chart points (Lagna + 7 grahas, excluding
 * Rahu/Ketu) occupy that element (neither absent nor overcrowded —
 * ideal even split would be ~2 each of 9 across 4 elements). Akash has
 * no zodiac sign of its own classically, so it's checked via Jupiter's
 * dignity/strength (wisdom/expansiveness = Akash's classical
 * signification) — same convention as the prior version of this file.
 */
function scoreP(planetaryPositions, shadbala) {
  ...
  ['Fire', 'Earth', 'Air', 'Water'].forEach(el => {
    if (counts[el] === 2) score += 2; // tightened: ideal balance is exactly 2 per element (8 points / 4)
  });

  const jupiterPos = planetaryPositions.jupiter;
  const jupiterExaltedOrOwn = jupiterPos && (EXALTATION.jupiter === jupiterPos.rashi || (OWN_SIGNS.jupiter || []).includes(jupiterPos.rashi));
  const jupiterRatio = shadbala && shadbala.jupiter && shadbala.jupiter.ratio;
  if (jupiterExaltedOrOwn || (jupiterRatio != null && jupiterRatio >= 1)) score += 2;
...
function calculateCEI(parsedData) {
  ...
  const moonNakshatraName = planetaryPositions && planetaryPositions.moon ? planetaryPositions.moon.nakshatra : null;
  ...
  const H = scoreH(bhavaBala, shadbala);
