...
function scoreD9(planetaryPositions) {
  ...
  const lord = SIGN_LORD[d1LagnaRashi]; // Lagna lord is determined by the REAL (D1) Lagna
  const lordNavPos = navPositions[lord];
  const lordD1Pos = planetaryPositions[lord];
  if (!lordNavPos || !lordD1Pos) return null;
  const lordDegree = lordD1Pos.degreeInSign || 0;

  const lordD9House = houseOfPlanet(lordNavPos.rashi, d9LagnaRashi);
  const dignity = dignityEffect(lord, lordNavPos.rashi, lordDegree);
  let score = 0;

  // 1. Lagna lord exalted or own-sign in D9 (degree-checked; partial credit at partial exaltation)
  if (dignity === 'full-exalted' || dignity === 'own') score += 2;
  else if (dignity === 'partial-exalted') score += 1;

  // 2. Kendra/trikona placement in D9 (from D9 Lagna)
  if (lordD9House != null && KENDRA_TRIKONA.includes(lordD9House)) score += 2;

  // 3. Free from malefic conjunction/aspect within D9
  const occupyingD9 = Object.keys(navPositions).filter(p => p !== 'lagna' && p !== lord
    && houseOfPlanet(navPositions[p].rashi, d9LagnaRashi) === lordD9House);
  const aspectingD9 = lordD9House != null ? planetsAspectingHouse(lordD9House, navPositions, d9LagnaRashi) : [];
  const maleficsD9 = new Set([...occupyingD9, ...aspectingD9]);
  if (![...maleficsD9].some(p => NATURAL_MALEFICS.includes(p))) score += 2;

  // 4. Benefic aspect on D9 Lagna (1st house of Navamsha)
  ...
  // 5. Not (degree-)debilitated, not combust in D1 (partial credit at partial debilitation)
  const combust = planetaryPositions.sun ? isCombust(lordD1Pos, planetaryPositions.sun) : false;
  if (dignity === 'full-debilitated') {
    // no points
  } else if (dignity === 'partial-debilitated') {
    if (!combust) score += 1;
  } else if (!combust) {
    score += 2;
  }
...
// ============================================================
// P — REBUILT with elemental trikona sign-matching
// ============================================================
...
/**
 * P = elemental balance. Confirmed rule: each trikona (Fire/Earth/
 * Air/Water) is "balanced" only if ALL THREE of its houses are
 * occupied by a rashi of that same element. 4 groups x 2 pts = 8.
 * 5th sub-criterion NOT YET CONFIRMED with father — placeholder
 * below gives 1 flat point pending clarification; flagged in detail.
 */
function scoreP(planetaryPositions) {
  ...
  TRIKONA_GROUPS.forEach(g => {
    ...
    if (matches === 3) score += 2; // fully balanced only if all 3 houses match
  });

  // UNCONFIRMED placeholder for the 5th criterion (see note above).
  score += 1;
