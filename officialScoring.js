...
function scoreL(planetaryPositions) {
  ...
  criteria.push({ label: 'Lagna lord is exalted or in its own sign', labelHi: 'लग्नेश उच्च या स्वराशि में है', passed: EXALTATION[lord] === lordPos.rashi || (OWN_SIGNS[lord] && OWN_SIGNS[lord].includes(lordPos.rashi)) });
  criteria.push({ label: 'Lagna lord is placed in a kendra/trikona house', labelHi: 'लग्नेश केंद्र/त्रिकोण भाव में स्थित है', passed: lordHouse != null && KENDRA_TRIKONA.includes(lordHouse) });
  ...
  criteria.push({ label: "Lagna lord's house is free from malefic occupation/aspect", labelHi: 'लग्नेश का भाव पाप ग्रहों की युति/दृष्टि से मुक्त है', passed: ![...malefics].some(p => NATURAL_MALEFICS.includes(p)) });

  const aspectingLagna = planetsAspectingHouse(1, planetaryPositions, lagnaRashi);
  criteria.push({ label: 'A natural benefic (Jupiter/Venus/Mercury) aspects the Lagna', labelHi: 'एक शुभ ग्रह (गुरु/शुक्र/बुध) लग्न पर दृष्टि डालता है', passed: aspectingLagna.some(p => NATURAL_BENEFICS.includes(p)) });

  const combust = planetaryPositions.sun ? isCombust(lordPos, planetaryPositions.sun) : false;
  criteria.push({ label: 'Lagna lord is not combust and not debilitated', labelHi: 'लग्नेश अस्त नहीं है और नीच राशि में नहीं है', passed: !combust && DEBILITATION[lord] !== lordPos.rashi });
...
function scoreS(planetaryPositions) {
  ...
  criteria.push({ label: 'Sun is exalted or in its own sign', labelHi: 'सूर्य उच्च या स्वराशि में है', passed: EXALTATION.sun === sunPos.rashi || (OWN_SIGNS.sun && OWN_SIGNS.sun.includes(sunPos.rashi)) });
  criteria.push({ label: 'Sun is in the 10th house or a kendra/trikona (1/5/9)', labelHi: 'सूर्य दशम भाव या केंद्र/त्रिकोण (1/5/9) में है', passed: sunHouse === 10 || [1, 5, 9].includes(sunHouse) });
  ...
  criteria.push({ label: 'Sun is influenced by a natural benefic (Jupiter/Venus)', labelHi: 'सूर्य पर एक शुभ ग्रह (गुरु/शुक्र) का प्रभाव है', passed: [...influencingSun].some(p => p === 'jupiter' || p === 'venus') });
  criteria.push({ label: 'Sun is free from Rahu/Ketu and Mars/Saturn influence', labelHi: 'सूर्य राहु/केतु और मंगल/शनि के प्रभाव से मुक्त है', passed: ![...influencingSun].some(p => p === 'rahu' || p === 'ketu') && ![...influencingSun].some(p => p === 'mars' || p === 'saturn') });
  criteria.push({ label: 'Sun is not in a dusthana house (6/8/12) and not debilitated', labelHi: 'सूर्य दुष्ट भाव (6/8/12) में नहीं है और नीच राशि में नहीं है', passed: !DUSTHANA.includes(sunHouse) && DEBILITATION.sun !== sunPos.rashi });
...
function scoreM(planetaryPositions) {
  ...
  criteria.push({ label: "Moon's tithi distance from Sun is in the favorable range", labelHi: 'सूर्य से चंद्रमा की तिथि दूरी अनुकूल सीमा में है', passed: tithiDist >= 50 && tithiDist <= 180 });

  criteria.push({ label: 'Moon is exalted or in its own sign', labelHi: 'चंद्रमा उच्च या स्वराशि में है', passed: EXALTATION.moon === moonPos.rashi || (OWN_SIGNS.moon && OWN_SIGNS.moon.includes(moonPos.rashi)) });
  ...
  criteria.push({ label: 'Moon is influenced by a natural benefic', labelHi: 'चंद्रमा पर एक शुभ ग्रह का प्रभाव है', passed: [...influencingMoon].some(p => NATURAL_BENEFICS.includes(p)) });

  criteria.push({ label: 'Moon is in a kendra/trikona house, free from malefic aspects', labelHi: 'चंद्रमा केंद्र/त्रिकोण भाव में है, पाप दृष्टि से मुक्त', passed: moonHouse != null && KENDRA_TRIKONA.includes(moonHouse) && ![...influencingMoon].some(p => NATURAL_MALEFICS.includes(p)) });
  ...
  criteria.push({ label: 'Moon is not debilitated, not conjunct malefics, and not combust', labelHi: 'चंद्रमा नीच राशि में नहीं, पाप ग्रहों के साथ नहीं, और अस्त नहीं है', passed: DEBILITATION.moon !== moonPos.rashi && !occupyingMoonHouse.some(p => NATURAL_MALEFICS.includes(p)) && !combust });
...
function scoreN(shadbala, nakshatraLord, planetaryPositions, moonNakshatraName, bhavaBala) {
  ...
  if (isGrahaLord) {
    ...
    criteria.push({ label: 'Nakshatra lord is strong (Shadbala ratio >=1)', labelHi: 'नक्षत्रेश बलवान है (षड्बल अनुपात >=1)', passed: ratio != null && ratio >= 1 });
  } else {
    const houseStrength = lordHouse != null && bhavaBala && bhavaBala[lordHouse] && bhavaBala[lordHouse].strengthPercent;
    criteria.push({ label: 'Nakshatra lord (shadow planet) is strong (house Bhava Bala >=50%)', labelHi: 'नक्षत्रेश (छाया ग्रह) बलवान है (भाव बल >=50%)', passed: !!(houseStrength != null && houseStrength >= 50) });
  }

  criteria.push({ label: 'Nakshatra lord is placed in a kendra/trikona house', labelHi: 'नक्षत्रेश केंद्र/त्रिकोण भाव में स्थित है', passed: !!(lordPos && lordHouse != null && KENDRA_TRIKONA.includes(lordHouse)) });
  ...
  if (lordPos) {
    ...
    criteria.push({ label: 'Nakshatra lord is free from malefic influence', labelHi: 'नक्षत्रेश पाप ग्रहों के प्रभाव से मुक्त है', passed: ![...malefics].some(p => NATURAL_MALEFICS.includes(p)) });
  } else {
    criteria.push({ label: 'Nakshatra lord is free from malefic influence', labelHi: 'नक्षत्रेश पाप ग्रहों के प्रभाव से मुक्त है', passed: false });
  }

  criteria.push({ label: "Moon's nakshatra is of a benign nature", labelHi: 'चंद्रमा का नक्षत्र शुभ प्रकृति का है', passed: !!(moonNakshatraName && !SEVERELY_INAUSPICIOUS_NAKSHATRAS.includes(normalizeNakshatraName(moonNakshatraName))) });
  ...
  criteria.push({ label: 'Nakshatra lord is exalted / in its own sign', labelHi: 'नक्षत्रेश उच्च / स्वराशि में है', passed: exaltPassed });
...
function scoreD9(planetaryPositions) {
  ...
  criteria.push({ label: 'D9 Lagna is benefic-linked (occupied/aspected by a benefic, or itself an exaltation sign)', labelHi: 'नवांश लग्न शुभ-संबंधित है (शुभ ग्रह से युत/दृष्ट, या स्वयं उच्च राशि)', passed: beneficNearD9Lagna || isExaltationSignForSomePlanet });
  ...
  criteria.push({ label: 'D9 lord (Navamshesh) is strong (not retrograde, not conjunct malefic, average degree)', labelHi: 'नवांशेश बलवान है (वक्री नहीं, पाप ग्रह के साथ नहीं, औसत अंश)', passed: d9LordStrong });

  // 3. Sun strong -- father's confirmed definition, checked in D1
  const sunStrongInD9 = isPlanetStrongD1('sun', planetaryPositions);
  criteria.push({ label: 'Sun is strong (not retrograde, not conjunct malefic, average degree)', labelHi: 'सूर्य बलवान है (वक्री नहीं, पाप ग्रह के साथ नहीं, औसत अंश)', passed: sunStrongInD9 });
  ...
  criteria.push({ label: 'Moon is Vargottama, or strong and malefic-free in D9', labelHi: 'चंद्रमा वर्गोत्तम है, या नवांश में बलवान और पाप-मुक्त है', passed: moonStrong });
  ...
  criteria.push({ label: "D1 Lagna lord is not fully debilitated in D9 (yoga protected)", labelHi: 'जन्म लग्नेश नवांश में पूर्ण नीच नहीं है (योग-रक्षित)', passed: yogaProtected });
...
function scoreP(planetaryPositions) {
  ...
  TRIKONA_GROUPS.forEach(g => {
    ...
    criteria.push({ label: `${g.element} trikona (houses ${g.houses.join('/')}) all belong to the ${g.element} element`, labelHi: `${ELEMENT_HI[g.element] || g.element} त्रिकोण (भाव ${g.houses.join('/')}) सभी ${ELEMENT_HI[g.element] || g.element} तत्व के हैं`, passed: matches === 3 });
  });

  const akashaScore = scoreAkasha(planetaryPositions);
  criteria.push({ label: "Akasha (Ether): Jupiter is well-placed (kendra/trikona) and not debilitated", labelHi: 'आकाश (ईथर): गुरु शुभ स्थान (केंद्र/त्रिकोण) में है और नीच राशि में नहीं है', passed: akashaScore === 2 });
...
function scoreH(ashtakvarga, planetaryPositions) {
  ...
  AV_TRIKONA_GROUPS.forEach(g => {
    ...
    criteria.push({ label: `Houses ${g.houses.join('-')} group has no weak house (bindus <= ${WEAK_BINDU_THRESHOLD})`, labelHi: `भाव ${g.houses.join('-')} समूह में कोई कमजोर भाव नहीं (बिंदु <= ${WEAK_BINDU_THRESHOLD})`, passed: !anyWeak });
  });
  ...
  criteria.push({ label: 'Body-significator planet (Lagna lord) is stable (kendra/trikona, not debilitated)', labelHi: 'शरीर-कारक ग्रह (लग्नेश) स्थिर है (केंद्र/त्रिकोण, नीच राशि में नहीं)', passed: significatorStable });
...
function calculateCEI(parsedData) {
  ...
  const components = {};
  const reasons = {};
  // NEW: full per-criterion breakdown (every criterion, not just failed
  // ones) with the marks obtained for each -- this is what the PDF's
  // प्राप्त अंक (marks obtained) column reads from, one entry per CEI
  // factor, mirroring the shape lpiEngine.js already returns for LPI.
  const criteriaByFactor = {};
  Object.entries(results).forEach(([key, r]) => {
    components[key] = r ? r.score : null;
    if (r && Array.isArray(r.criteria)) {
      const failed = r.criteria.filter(c => !c.passed).map(c => ({ labelHi: c.labelHi || '', labelEn: c.label }));
      if (failed.length) reasons[key] = failed;
      criteriaByFactor[key] = r.criteria.map(c => ({
        labelHi: c.labelHi || '',
        labelEn: c.label,
        score: c.passed ? 2 : 0,
        maxScore: 2,
  ...
  if (Array.isArray(elResult.criteria)) {
    const activeDoshas = elResult.criteria.filter(c => !c.passed).map(c => ({ labelHi: c.labelHi || '', labelEn: c.label }));
    if (activeDoshas.length) reasons.EL = activeDoshas;
    // EL's "score" per dosha is a DEDUCTION (2 points lost when present),
    // the inverse sense of every other factor's "marks obtained" -- kept
    // as its own field so the PDF can label this column differently.
    criteriaByFactor.EL = elResult.criteria.map(c => ({
      labelHi: c.labelHi || '',
      labelEn: c.label,
      deduction: c.passed ? 0 : 2,
      maxDeduction: 2
    }));
  }
  ...
  if (missing.length) {
    return {
      ...
      detail: { reason: `Missing data for: ${missing.join(', ')}`, components, EL, reasons, criteriaByFactor }
    };
  }
  ...
  return {
    ...
    detail: {
      ...
      reasons,
      // Full per-criterion breakdown (all 5 criteria per factor, marks obtained for each) -- what the PDF's प्राप्त अंक column reads.
      criteriaByFactor,
      confirmationStatus: {
